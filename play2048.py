...
def start_screen():
    ...
    color_dark = (100, 100, 100)
    smallfont=pygame.font.SysFont('Corbel', 35)
    text = smallfont.render("Начать", True, (255, 255, 255))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
	            ...
	            if width / 2 <= mouse[0] <= width / 2 + 160 and height / 2 <= mouse[1] <= height / 2 + 60:
                        return
        ...
        mouse = pygame.mouse.get_pos()
        if width / 2 <= mouse[0] <= width / 2 + 160 and height / 2 <= mouse[1] <= height / 2 + 60:
	        pygame.draw.rect(screen, color_light, [width / 2, height / 2, 160, 60])
        else:
	        pygame.draw.rect(screen, color_dark, [width / 2, height / 2, 160, 60])
        if width2 / 2 <= mouse[0] <= width2 / 2 + 500 and height2 / 2 <= mouse[1] <= height2 / 2 + 60:
